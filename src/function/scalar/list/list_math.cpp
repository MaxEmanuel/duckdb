...
static void ListMathFunction(DataChunk &args, ExpressionState &state, Vector &result) {
...
static unique_ptr<FunctionData> ListMathBind(ClientContext &context, ScalarFunction &bound_function,
...
static unique_ptr<BaseStatistics> ListMathStats(ClientContext &context, FunctionStatisticsInput &input) {
...
ScalarFunction ListMathFun::GetFunction() {
	// the arguments and return types are actually set in the binder function
	auto fun = ScalarFunction({LogicalType::LIST(LogicalType::ANY), LogicalType::LIST(LogicalType::ANY)},
	                          LogicalType::LIST(LogicalType::ANY), ListMathFunction, ListMathBind, nullptr,
	                          ListMathStats);
	fun.null_handling = FunctionNullHandling::SPECIAL_HANDLING;
	return fun;
}

void ListMathFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction({"list_add"}, GetFunction());
}
